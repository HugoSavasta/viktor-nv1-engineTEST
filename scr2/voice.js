...
import { transposeValue, transposeParam } from "viktor-nv1-settings-convertor";
import { CUSTOM_WAVEFORMS, FILTER_FREQUENCY_UPPER_BOUND, FILTER_FREQUENCY_LOWER_BOUND, LFO_DEFAULT_RATE, LFO_DEFAULT_FORM, LFO_DEFAULT_FREQUENCY_CENTER, LFO_DEFAULT_FREQUENCY_RANGE, MODULATION_LFO_FREQUENCY_RANGE, FAKE_ZERO, OSC_WAVEFORM } from "./conste";
import OscillatorBank from "./oscillator-bank";
import WaveformSource from "./waveform-source";
...
function Voice( audioContext ) {
	var self = this,
		...
		waveformSource = new WaveformSource( audioContext, CUSTOM_WAVEFORMS ),
		...
		filterEnvelope = new Envelope( audioContext, "frequency", FILTER_FREQUENCY_UPPER_BOUND, FILTER_FREQUENCY_LOWER_BOUND ),
		filterLfo = new LFO( audioContext, [ lfoControlledFilter.node ], "frequency", {
			rate: LFO_DEFAULT_RATE,
			defaultForm: LFO_DEFAULT_FORM,
			centerFrequency: LFO_DEFAULT_FREQUENCY_CENTER,
			frequencyRange: LFO_DEFAULT_FREQUENCY_RANGE
		} ),
	...
	var modulationLfo = new LFO( audioContext, oscillatorBank.oscillators, "detune", {
		rate: 0,
		defaultForm: LFO_DEFAULT_FORM,
		frequencyRange: MODULATION_LFO_FREQUENCY_RANGE
	} );
...
Voice.prototype = {
	...
	onPitchBend: function( pitchBend ) {
		...
		self.pitchSettings = {
			bend: transposeParam( pitchBend, self.settings.pitch.bend.range )
		};
	},
			...
			set: function( settings ) {
				...
				if ( !oldSettings || ( oldSettings.waveform.value !== settings.waveform.value ) ) {
					...
					modulationLfo.waveform = {
						...
						customFormFFT: waveformSource.customForms[ OSC_WAVEFORM[ index ] ]
					};
				}
				...
				oscillatorBank.forEach( function( osc, index ) {
					...
					if ( !oldSettings || oldOscSettings.waveform.value !== newOscSettings.waveform.value ) {
						...
						if ( defaultForm ) {
							...
							osc.customWaveform = waveformSource.customForms[ OSC_WAVEFORM[ waveform ] ];
						}
					}
